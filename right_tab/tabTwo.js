...
export function initializeTabTwo(root = document) {
  ...
  function renderFarmSummary(alpacaCount, customizations, assignments) {
    ...
    elements.farmSummary.textContent = `Your farm has ${alpacaCount} alpaca${alpacaCount === 1 ? "" : "s"}, ${customizations.length} customization item${customizations.length === 1 ? "" : "s"}, and ${rewardCounts.Gold} Gold, ${rewardCounts.Silver} Silver, ${rewardCounts.Bronze} Bronze reward${rewardCounts.Gold + rewardCounts.Silver + rewardCounts.Bronze === 1 ? "" : "s"}.`;
    elements.customizationList.innerHTML = customizations.length
      ? customizations.map((item) => `<span class="customization-tag">${escapeHtml(item)}</span>`).join("")
